...
int main(int argc, char ** argv){
  ...
  if (myid == 0)
	...
	for(j=0;j<N;j++){			
      ...
      MPI_Send(b, N*N, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      printf("J'envoie b\n");

      // envoyer la tranche de chacun
      for(i=1; i<numprocs; i++) {
	      MPI_Send(&a[i*tranche][0], N*tranche, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      }
      printf("J'envoie la tranche de chacun\n");

      calculer(a,b,c,tranche);

      // recevoir la tranche de chacun
      for(i=1; i<numprocs; i++) {
	      MPI_Recv(&c[i*tranche][0], N*tranche, MPI_CHAR, 1, 0, MPI_COMM_WORLD, &status);
      }
      printf("Je recois la tranche de chacun\n");
  ...
  else
    {
      ...
      // recevoir a et b
      MPI_Recv(b, N*N, MPI_CHAR, 1, 0, MPI_COMM_WORLD, &status);
      printf("Je recois b\n");
      MPI_Recv(a, tranche*N, MPI_CHAR, 1, 0, MPI_COMM_WORLD, &status);
      printf("Je recois a\n");

      calculer(a,b,c,tranche);

     // envoyer c
      MPI_Send(c, tranche*N, MPI_CHAR, 1, 0, MPI_COMM_WORLD);
      printf("J'envoie c\n");
