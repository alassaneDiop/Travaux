// pour compiler : gcc -o3 -fopenmp -lm tsp-main.c
...
void tspParallel (int hops, int len, int *path, int mask)
{
 int i ;
 int me, dist ;
...
//	   		path [hops] = i ;
	   		dist = distance[me][i] ;
	   		tspParallel (hops+1, len+dist, mypath,  mask | (1 << i)) ;
	   	}
       }
     }
   }
