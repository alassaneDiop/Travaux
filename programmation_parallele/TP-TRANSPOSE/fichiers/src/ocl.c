...
void ocl_init (void)
{
  ...
  if (TILE > 16)
    exit_with_error ("TILE (%d) cannot exceed 16", TILE);
  ...
  // Get list of devices
  //
  err = clGetDeviceIDs (pf[platform_no], CL_DEVICE_TYPE_GPU, MAX_DEVICES, devices, &nb_devices);
  ...
  //
  {
    ...
    sprintf (flags,
	     "-cl-mad-enable -cl-fast-relaxed-math -DDIM=%d -DSIZE=%d -DTILE=%d",
	     DIM, SIZE, TILE);
...
void ocl_send_image (unsigned *image)
{
  ...
  clFinish (queue);
  printf ("Initial image sent to device.\n");
}
